...
def broadcast_summary(room_id, summary, uploader_name, pdf_name):
    # Singleton Pattern: channel_layer is the single instance for messaging
    channel_layer = get_channel_layer()
    logger.info(f"Broadcasting summary to group room_{room_id}: {summary[:50]}... by {uploader_name}")
    # Observer Pattern: Broadcasting summary to all room participants (observers)
...
def broadcast_summarizing_start(room_id):
    ...
    # Observer Pattern: Broadcasting event to all room participants (observers)
    async_to_sync(channel_layer.group_send)(
        f'room_{room_id}',
        {
...
def broadcast_chunk_summary(room_id, summary, chunk_number):
    # Singleton Pattern: channel_layer is the single instance for messaging
    channel_layer = get_channel_layer()
    # Observer Pattern: Broadcasting chunk summary to all room participants (observers)
    async_to_sync(channel_layer.group_send)(
        f'room_{room_id}',
        {
...
# Facade Pattern: Simplifies interaction with PyPDF2, fitz, pytesseract, and transformers for text extraction
...
# Decorator Pattern: Framework-level decorator prevents caching
@never_cache
...
# Decorator Pattern: Framework-level decorator adds POST-only restriction
@require_POST
    ...
    if all([department, course, topic, date_str, time_str, visibility]):
        ...
        try:
            ...
            try:
                # Singleton Pattern: channel_layer is the single instance for messaging
                channel_layer = get_channel_layer()
                # Observer Pattern: Broadcasting notification to all room participants (observers)
                async_to_sync(channel_layer.group_send)(
                    f'room_{room.id}',
                    {
