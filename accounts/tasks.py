...
def generate_quiz_questions(room, key_points=None):
    ...
    # Strategy Pattern: QUESTION_TEMPLATES provides interchangeable question generation algorithms based on course
    ...
    for i in range(min(num_questions, len(key_points))):
        ...
        # Strategy Pattern: Dynamically selecting a question generation strategy
        ...
        if callable(question_template["correct_answer"]):
            # Strategy Pattern: Correct answer logic varies by selected strategy (e.g., lambda for true_false)
        ...
        if question_type == "true_false":
            ...
            if course == "Algorithms" and question_type == "time_complexity":
                # Strategy Pattern: Specialized strategy for time_complexity questions
                ...
                # Strategy Pattern: Specialized strategy for use_case questions
...
def start_quiz_for_room(room_id):
    ...
    try:
        ...
        try:
            # Singleton Pattern: channel_layer is a single global instance provided by Channels
            channel_layer = get_channel_layer()
            if channel_layer is None:
                logger.error("Channel layer is None. Cannot send quiz start notification.")
                return
            # Observer Pattern: Notifying all room participants (observers) of quiz start
